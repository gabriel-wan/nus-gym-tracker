...
| Messages use `parse_mode: HTML` | Bold copy needs a parse mode, and monospace is the only way to align `/history` columns - Telegram's default font is proportional, so `1` is narrower than `0` and `11:00` is shorter than `07:00`. HTML over MarkdownV2 because it escapes three characters rather than eighteen. |
| `<code>` for charts, not `<pre>` | Both give monospace. `<pre>` is Telegram's *code block*: grey chrome and a copy button, which is wrong for a chart. |
| Bars are 16 characters wide, filled blocks only | At 10 everything between 15% and 25% collapsed into the same two or three blocks. 16 keeps the line near 29 characters, which fits a phone without sideways scrolling. No empty-track glyph: `░` is a different width from `█` in some fonts, which silently skewed every row. |
